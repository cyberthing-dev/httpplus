...
def _421(path:str="") -> str:
    """Returns the HTML for the 421 error page."""
    message = path if path else "The requested `Host` header"
...
def _426(path:str="") -> str:
    """Returns the HTML for the 426 error page."""
    message = path if path else "this endpoint"
...
def _500(path:str="") -> str:
    """Returns the HTML for the 500 error page."""
    message = path if path else "The page you were looking for"
    return generate_html(500,"Internal Server Error",f"{message} experienced an error during internal processing.")

def _501(path:str="") -> str:
